...
from Order.order import OrderModel, OrderSchema, OrderUpdateSchema, StatusUpdateSchema
...
async def get_orders(
    ...
    if not store_id:
        raise HTTPException(
            ...
            detail="Store ID não encontrado no token. Apenas funcionários de lojas podem acessar pedidos."
        )
...
@router.get("/me/{customer_id}", status_code=status.HTTP_200_OK)
@save_log(AuditActionEnum.READ, OrderModel)
...
@router.post("/{store_id}/{customer_id}", status_code=status.HTTP_201_CREATED)
...
async def create_order(
    ...
    return await service.create_for_customer(customer_id, store_id, order_data)

@router.patch("/{order_id}/status", status_code=status.HTTP_200_OK)
@require_roles(["delivery", "chef", "waiter"])
@save_log(AuditActionEnum.UPDATE, OrderModel)
async def update_status(
        request: Request,
        order_id: str,
        order_data: StatusUpdateSchema,
        current_user: dict = Depends(get_current_user),
        service: OrderService = Depends(get_order_service)
) -> OrderModel:
    store_id = request.state.store_id
    return await service.update_order_status(order_id, order_data.status, store_id)
...
async def update_order(
    ...
    store_id = request.state.store_id
    return await service.update_order_items(order_id, store_id, order_data.items)
