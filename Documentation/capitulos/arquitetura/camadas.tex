...
\begin{enumerate}
  ...
  e comportamentais conforme definido por gamma2009design.

  \item \textbf{Padrões Empresariais}: O padrão \textit{Repository} e \textit{Data Transfer Object (DTO)}
  são padrões de aplicação empresarial descritos por fowler2015patterns que facilitam a separação
  ...
  em uma camada dedicada, seguindo arquiteturas em camadas conforme descrito por hohpe2015enterprise
  e evans2014domain.
\end{enumerate}
...
\begin{description}
  ...
  método de pagamento, tipo de entrega e vericação de status do pedido.
\end{description}
...
\begin{itemize}
  \item \texttt{RoleEnum}: Define os papéis de acesso de funcionários (manager, chef, waiter, delivery).
...
\hspace{0.5cm} Todas as entidades implementam soft delete através do campo \texttt{deleted\_at}. Isso preserva integridade referencial
